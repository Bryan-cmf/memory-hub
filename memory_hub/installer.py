...
def install_qdrant():
    ...
    try:
        r = subprocess.run(["curl", "-sf", "http://localhost:6333/health"],
                          capture_output=True, timeout=3)
        if r.returncode == 0:
            print(f"  {ok('Qdrant already running')}")
            return True
    except Exception:
        pass
    ...
    try:
        ...
        print(f"  {err('Docker not available. Please install Docker first.')}")
        print(f"    {dim('https://docs.docker.com/get-docker/')}")
        return False
    ...
    if status:
        ...
        print(f"  Creating Qdrant container...")
        subprocess.run(
    ...
    time.sleep(3)
    try:
        r2 = subprocess.run(["curl", "-sf", "http://localhost:6333/health"],
    ...
    print(f"  {warn('Qdrant may need more time to start')}")
...
OPTIONAL_BACKENDS = {
    "chroma": {
        ...
        "verify": lambda: __import__("chromadb"),
    },
    "lancedb": {
        "name": "LanceDB",
        "desc": "Embedded vector DB (pip, lightest)",
        "install": lambda: subprocess.run(
            [sys.executable, "-m", "pip", "install", "--quiet", "lancedb"],
            capture_output=True, timeout=60
        ),
        "verify": lambda: __import__("lancedb"),
    },
}
...
def install_optional():
    ...
    selected = {}
    for i, (key, info) in enumerate(OPTIONAL_BACKENDS.items(), 1):
        try:
            info["verify"]()
    ...
    print(f"  [Enter] Skip all optional backends")
    print()

    while True:
        try:
            choice = input(f"  Your choice (1-{len(OPTIONAL_BACKENDS)}, Enter to skip): ").strip()
            if not choice:
                break
            idx = int(choice)
            if 1 <= idx <= len(OPTIONAL_BACKENDS):
                key = list(OPTIONAL_BACKENDS.keys())[idx - 1]
                ...
                if key in selected:
                    ...
                    try:
                        ...
                        print(f"  {warn(info['name'] + ' installed but verification failed')}")
                break
            else:
            ...
            if choice.lower() == 'q':
                break
            print(f"  {err('Enter a number or press Enter to skip')}")
